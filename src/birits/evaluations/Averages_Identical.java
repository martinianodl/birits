 ...
 * @author martinianodl
 */
public class Averages_Identical {

    public static void main(String[] args) throws IOException {
        try (BufferedWriter csvFinal = new BufferedWriter(new FileWriter("averages_identical"
                + ".csv"))) {
            csvFinal.write("part,num,MRR_A@1,MRR_F@1,MRR_A@5,MRR_F@5,MRR_A@10,MRR_F@10,MRR_A@25,MRR_F@25");
            csvFinal.newLine();
            File folder = new File("RESULT IDENTICAL/");
            ...
            for (File listOfFile : listOfFiles) {
                ...
                if (listOfFile.isFile() && listOfFile.getName().contains(".csv")) {
                    ...
                    try (BufferedReader buffRead = new BufferedReader(f1)) {
                        ...
                        System.out.println(splitFile[0] + ", " + splitFile[1].substring(0, splitFile[1].length() - 4) + ", " + calculateAverage(MRR_A1) + ", " + calculateAverage(MRR_F1) + ", " + calculateAverage(MRR_A5) + ", " + calculateAverage(MRR_F5) + ", " + calculateAverage(MRR_A10) + ", " + calculateAverage(MRR_F10) + ", " + calculateAverage(MRR_A25) + ", " + calculateAverage(MRR_F25));
                        csvFinal.write(splitFile[0] + ", " + splitFile[1].substring(0, splitFile[1].length() - 4) + ", " + calculateAverage(MRR_A1) + ", " + calculateAverage(MRR_F1) + ", " + calculateAverage(MRR_A5) + ", " + calculateAverage(MRR_F5) + ", " + calculateAverage(MRR_A10) + ", " + calculateAverage(MRR_F10) + ", " + calculateAverage(MRR_A25) + ", " + calculateAverage(MRR_F25));
                        csvFinal.newLine();
                    }
                }
            }
